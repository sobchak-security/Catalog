  ...
  build:
    ...
    steps:
      ...
      - name: Build tool binaries once
        id: tools
        run: |
          swift build --package-path tools -c release --product catalog-build
          swift build --package-path tools -c release --product catalog-sign
          swift build --package-path tools -c release --product catalog-publish

          BIN_DIR=$(swift build --package-path tools -c release --show-bin-path)
          echo "bin_dir=$BIN_DIR" >> "$GITHUB_OUTPUT"

          test -x "$BIN_DIR/catalog-build"
          test -x "$BIN_DIR/catalog-sign"
          test -x "$BIN_DIR/catalog-publish"
      - name: Build snapshot
        id: meta
        run: |
          REV=${GITHUB_REF_NAME#catalog-v}
          "${{ steps.tools.outputs.bin_dir }}/catalog-build" \
      ...
      - uses: actions/upload-artifact@v4
        with:
          ...
          path: |
            ${{ steps.tools.outputs.bin_dir }}/catalog-build
            ${{ steps.tools.outputs.bin_dir }}/catalog-sign
            ${{ steps.tools.outputs.bin_dir }}/catalog-publish
          retention-days: 7
